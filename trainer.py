...
from dataloader import get_loader, gt_to_tensor
from util.utils import AvgMeter
from util.metrics import Evaluation_metrics
from util.losses import Optimizer, Scheduler, Criterion, Criterion_edge
from model.PEN import PEN


class Trainer():
    def __init__(self, args, save_path):
        self.writer = SummaryWriter(save_path + 'summary')
        super(Trainer, self).__init__()
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.size = args.img_size

        self.tr_img_folder = os.path.join(args.data_path, args.dataset, 'Train/Image')
        self.tr_gt_folder = os.path.join(args.data_path, args.dataset, 'Train/GT_Object')
        self.tr_edge_folder = os.path.join(args.data_path, args.dataset, 'Train/GT_Edge')
        # pdb.set_trace()
        self.train_loader = get_loader(self.tr_img_folder, self.tr_gt_folder, self.tr_edge_folder, train_size=args.img_size, phase='train',
                                       batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers,
                                       augmentation = args.augmentation, seed=args.seed)
        self.val_loader = get_loader(self.tr_img_folder, self.tr_gt_folder, self.tr_edge_folder, train_size=args.img_size, phase='val',
                                     batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers,
                                     augmentation = args.augmentation, seed=args.seed)

        # Network
        self.model = PEN(args).to(self.device)
        self.l_weight = [0.7,0.7,1.1,1.1,0.3,0.3,1.3]
        if args.multi_gpu:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            self.writer.add_scalar('lr', self.optimizer.state_dict()['param_groups'][0]['lr'], global_step=epoch)
            # Save models
            if val_loss < min_loss:
                ...
                self.writer.add_scalar('mae',best_mae,epoch)
                self.writer.add_scalar('loss',min_loss,epoch)

            else:
                early_stopping += 1
                
            if early_stopping == args.patience + 15:
                break
    ...
    def training(self, args):
        ...
        for images, masks, edges in tqdm(self.train_loader):
            ...
            # loss6 = self.criterion(ds_map[4], masks)
            # loss7 = self.criterion(ds_map[5], masks)
            # loss8 = self.criterion(ds_map[6], masks)
    ...
    def test(self, args, save_path):
        ...
        test_loader = get_loader(te_img_folder, te_gt_folder, edge_folder=None, train_size=args.img_size, phase='test',
...
class Tester():
    def __init__(self, args, save_path):
        ...
        # Network
        self.model = self.model = PEN(args).to(self.device)
        if args.multi_gpu:
            self.model = nn.DataParallel(self.model).to(self.device)
        ...
        if args.save_map is not None:
            os.makedirs(os.path.join('/Data/ZZY/P_Edge_N', 'snapshot', 'exp'+str(self.args.exp_num), self.args.dataset), exist_ok=True)
            ...
            for i, (images, masks, original_size, image_name) in enumerate(tqdm(self.test_loader)):
                ...
                for i in range(images.size(0)):
                    ...
                    if self.args.save_map is not None:
                        ...
                        # ds_map_3 = (ds_map[3][i].squeeze().detach().cpu().numpy()*255.0).astype(np.uint8)   # convert uint8 type
                        ...
                        # cv2.imwrite(os.path.join('/Data/ZZY/P_Edge_N', 'snapshot', \
                        #     'exp'+str(self.args.exp_num), 'dissertation_image', image_name[i]+'_map_3.png'), ds_map_3)
                        # cv2.imwrite(os.path.join('/Data/ZZY/P_Edge_N', 'snapshot', \
